...
local function create_realm(msg)
        ...
        if is_sudo(msg) or is_realm(msg) and is_admin(msg) then
                local group_creator = msg.from.print_name
                create_group_chat (group_creator, group_name, ok_cb, false)
                return 'اتاق فرمان [ '..string.gsub(group_name, '_', ' ')..' ] ساخته شد 😎.'
        end
end
...
local function get_group_type(msg)
  ...
  if data[tostring(msg.to.id)] then
    if not data[tostring(msg.to.id)]['group_type'] then
     return 'این گروه عنوان ندارد'
    end
     local group_type = data[tostring(msg.to.id)]['group_type']
     return group_type
  else 
     return 'عنوانِ این گپ یافت نشد'
  end 
end
...
local function set_description(msg, data, target, about)
    if not is_admin(msg) then
        return "این دستور مخصوص ادمین هاست"
    end
...
local function lock_group_name(msg, data, target)
    ...
    local group_name_lock = data[tostring(target)]['settings']['lock_name']
        if group_name_lock == 'yes' then
            return 'نامِ گروه از قبل قفل بوده'
        else
...
local function unlock_group_name(msg, data, target)
    ...
    local group_name_lock = data[tostring(target)]['settings']['lock_name']
        if group_name_lock == 'no' then
            return 'نام گروه از قبل باز بوده'
        else
...
local function lock_group_member(msg, data, target)
    ...
    local group_member_lock = data[tostring(target)]['settings']['lock_member']
        if group_member_lock == 'yes' then
            return 'اعضای گروه از قبل قفل بوده'
        else
...
local function lock_group_photo(msg, data, target)
    ...
    local group_photo_lock = data[tostring(target)]['settings']['lock_photo']
        if group_photo_lock == 'yes' then
            return 'عکس گروه از قبل قفل بوده.'
        else
            data[tostring(target)]['settings']['set_photo'] = 'waiting'
            save_data(_config.moderation.data, data)
        end
        return 'لطفا عکس جدید برای گروه ارسال کنید.'
end
...
local function lock_group_flood(msg, data, target)
    ...
    local group_flood_lock = data[tostring(target)]['settings']['flood']
        if group_flood_lock == 'yes' then
            return 'سیل گروه قفل است'
        else
            data[tostring(target)]['settings']['flood'] = 'yes'
            save_data(_config.moderation.data, data)
        return 'سیل گروه قفل شد'
        end
end
...
local function unlock_group_flood(msg, data, target)
    ...
    local group_flood_lock = data[tostring(target)]['settings']['flood']
        if group_flood_lock == 'no' then
            return 'سیل گروه قفل نیست'
        else
            data[tostring(target)]['settings']['flood'] = 'no'
            save_data(_config.moderation.data, data)
        return 'سیل گروه باز شد.'
        end
end
-- show group settings
...
local function admin_promote(msg, admin_id)
        ...
        if data[tostring(admins)][tostring(admin_id)] then
                return admin_name..' از قبل ادمین بوده.'
        end
...
local function admin_demote(msg, admin_id)
    ...
    local data = load_data(_config.moderation.data)
        ...
        save_data(_config.moderation.data, data)
        return admin_id..' از ادمین بودن برکنار شد.'
end
...
local function groups_list(msg)
    local data = load_data(_config.moderation.data)
        ...
        if not data[tostring(groups)] then
                return 'در حال حاضر هیچ گروهی موجود نیست.'
        end
...
local function realms_list(msg)
    local data = load_data(_config.moderation.data)
        ...
        if not data[tostring(realms)] then
                return 'در حال حاضر هیچ اتاق فرمانی موجود نیست.'
        end
...
local function admin_user_promote(receiver, member_username, member_id)
        ...
        if data['admins'][tostring(member_id)] then
                return send_large_msg(receiver, member_username..' از قبل ادمین بوده.')
        end
...
local function admin_user_demote(receiver, member_username, member_id)
    local data = load_data(_config.moderation.data)
        ...
        save_data(_config.moderation.data, data)
        return send_large_msg(receiver, 'Admin '..member_username..' حذف شد.')
end
